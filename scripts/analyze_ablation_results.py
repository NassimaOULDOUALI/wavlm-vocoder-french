...
METRICS = [
    'loss_g', 'loss_d', 'l1', 'mel', 'loss_stft',
    'loss_fm_mpd', 'loss_fm_msd', 'loss_gen_mpd', 'loss_gen_msd'
]
    ...
    try:
        ...
        for loc_name, state in locations:
            ...
            if debug:
                ...
                relevant_keys = [k for k in state.keys() if 'layer' in k.lower() or 'weight' in k.lower()]
            ...
            for key in state.keys():
                if 'layer_weights' in key.lower():
                    ...
                    if isinstance(logits, torch.Tensor):
                        ...
                        if debug:
                            print(f"  📊 Poids extraits: shape={weights.shape}, sum={weights.sum():.4f}")
                            print(f"      Top 3: {weights[:min(3, len(weights))]}")
                        
                        return weights
        
        if debug:
            print(f"  ⚠️  Aucun layer_weights trouvé dans aucun emplacement")
        return None
...
def analyze_n_layers_experiment(output_dir, n_values=range(1, 13), debug=True):
    """
    ...
    for n in n_values:
        ...
        if ckpt is None:
            print(f"❌ Aucun checkpoint valide")
            continue
        ...
        if weights is not None:
            ...
            print(f"⚠️  Pas de poids extraits")
...
def plot_ablation_curves(df, output_dir):
    ...
    if mel_col in df.columns and not df[mel_col].isna().all():
        ...
        print(f"✅ Figure: ablation_losses.pdf")
        plt.close()
    ...
    if weight_cols:
        ...
        print(f"✅ Figure: ablation_weights.pdf")
        plt.close()
